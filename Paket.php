...
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM paket") or die ($this->CekError());
...
public function detail($no_paket){
// 1. mysqli_query
$sql=$this->mysqli->query("SELECT * FROM paket WHERE no_paket='".$no_paket."'") or die ($this->CekError());
...
$this->mysqli->query("UPDATE paket SET ".$script_update." WHERE no_paket='".$no_paket."'") or die ($this->CekError());
// 4. tutup koneksi
$this->TutupKoneksi();
}
function hapus($no_paket){
// 1. Jalankan perintah delete query
$this->mysqli->query("DELETE FROM paket WHERE no_paket='$no_paket'");
// 2. tutup koneksi
$this->TutupKoneksi();
}
...
$sql_simpan="INSERT INTO paket (".$kolom_nya_baru.") VALUES (".$nilai_nya_baru.")";
